...
//			return true;
	...
	public static void addCDOfferings(CDOffering cdOffering) {
		CDOffering[] cdOfferingNew = new CDOffering[cdOfferings.length + 1];
		int i;
		for (i = 0; i < cdOfferings.length; i++) {
			cdOfferingNew[i] = cdOfferings[i];
		}
		cdOfferingNew[i] = cdOffering;
		MeritBank.cdOfferings = cdOfferingNew;
	}
